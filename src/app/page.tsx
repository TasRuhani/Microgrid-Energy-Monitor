...
export default function App() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (
        parseFloat(newPower) < lowEnergyThreshold &&
        parseFloat(newPower) > 0
      ) {
      ...
      if (
        parseFloat(jsonData.voltage) < lowVoltageThreshold ||
        parseFloat(jsonData.voltage) > highVoltageThreshold
      ) {
        newMetricStatus.voltage = "warning";
        newAlerts.push(
          "Voltage fluctuation detected. Maintenance may be required."
        );
      }
      ...
      if (parseFloat(newPf) < lowPfThreshold) {
        ...
        newAlerts.push(
          "Low power factor detected. Check for faulty loads or motors."
        );
      }
      ...
      if (parseFloat(newApparentPower) > parseFloat(newPower) * 1.5) {
        ...
        newAlerts.push(
          "Large difference between real and apparent power detected. This may indicate a problem."
        );
      }
      ...
      if (parseFloat(newReactivePower) > 100) {
        ...
        newAlerts.push(
          "High reactive power detected. This can lead to increased energy costs and grid instability."
        );
      }
      ...
      <div className="container mx-auto max-w-4xl py-8">
        ...
        {alerts.length > 0 &&
          alerts.map((alert, index) => (
            <div
              key={index}
              className="bg-red-600 text-white p-3 rounded-lg text-center mb-6 font-medium animate-pulse"
            >
              WARNING: {alert}
            </div>
          ))}
